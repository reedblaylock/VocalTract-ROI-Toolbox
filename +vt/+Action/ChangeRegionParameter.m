classdef ChangeRegionParameter < vt.Action
	...
	methods
		function this = ChangeRegionParameter(region, parameter, value, video)
			p = inputparser;
			...
			switch(parameter)
				...
				case {'origin', 'shape', 'radius', 'width', 'height'}
					region.(parameter) = value;
					
					region.mask = this.getMask(region, video);
		...
		function mask = getMask(this, region, video)
			...
			switch(shape)
				case 'Circle'
					mask = this.find_region(video.matrix, region.origin, region.radius);
				case 'Rectangle'
					mask = this.find_rectangle_region(video.matrix, region.origin, region.width, region.height);
				otherwise
