...
classdef Gui < vt.Root & vt.State.Listener
	...
	methods
		...
		function gui = createInterface(this)
			...
			gui.MidlineTab = vt.Component.Layout.VBox( ...
				gui.RightBox, ...
				'Padding', this.styles.Padding, ...
				'Spacing', this.styles.Spacing ...
			);
		...
		function gui = addMidlinePanelContents(this, gui)
			...
			gui.MidlineText = vt.Component.Text.Midline( ...
				gui.MidlineTab ...
			);
			
			% + Grid, for buttons
			gui.MidlineGrid = vt.Component.Layout.Grid( ...
				gui.MidlineTab, ...
				'Spacing', this.styles.Spacing + 2 ...
			);
		
		
			% 1-1 + Change color button
% 			gui.MidlineGrid1_1 = vt.Component.Button.MidlineColor( ...
% 				gui.RightBoxGrid, ...
% 				'Change color' ...
% 			);
			% 1-1 + Empty area (for spacing purposes)
			gui.MidlineGrid1_1 = vt.Component.Layout.Empty( ...
				gui.MidlineGrid ...
			);
			% 2-1 + Empty area (for spacing purposes)
			gui.MidlineGrid2_1 = vt.Component.Layout.Empty( ...
				gui.MidlineGrid ...
			);
			% 3-1 + Empty area (for spacing purposes)
			gui.MidlineGrid3_1 = vt.Component.Layout.Empty( ...
				gui.MidlineGrid ...
			);
			
			% 1-2 + Empty area (for spacing purposes)
			gui.MidlineGrid1_2 = vt.Component.Layout.Empty( ...
				gui.MidlineGrid ...
			);
			% 2-2 + New midline button
			gui.MidlineGrid2_2 = vt.Component.Button.NewMidline( ...
				gui.MidlineGrid, ...
				'New midline' ...
			);
			% 3-2 + Delete midline button
			gui.MidlineGrid3_2 = vt.Component.Button.DeleteMidline( ...
				gui.MidlineGrid, ...
				'Delete midline' ...
			);
		
			gui.RightBoxGrid.setParameters('Widths', [-1 -1], 'Heights', -1.*ones(1, 3));
			
			% Text space should be twice as big as button space so that buttons
			% are sized similarly to the ones in Region settings
			gui.MidlineTab.setParameters('Heights', [-2, -1]);
		end
		...
		function [] = onIsEditingChange(this, state)
			switch(state.isEditing)
				...
				case 'midlineNew'
					...
					% + 2-2 Empty button
					this.gui.MidlineGrid2_2 = vt.Component.Layout.Empty( ...
						this.gui.MidlineGrid ...
					);
					this.initializeComponent(this.gui.MidlineGrid2_2);
					
					% + 3-2 Cancel button
					this.gui.MidlineGrid3_2 = vt.Component.Button.CancelMidline( ...
						this.gui.MidlineGrid, ...
				...
				case 'midlineEdit'
					...
					% Delete buttons
					% + 1-1 Color button
					this.gui.MidlineGrid1_1 = vt.Component.Button.MidlineColor( ...
						this.gui.MidlineGrid, ...
						'Midline color' ...
					);
					this.initializeComponent(this.gui.MidlineGrid1_1);
					
					% + 1-2 Save midline button
					this.gui.MidlineGrid1_2 = vt.Component.Button.SaveMidline( ...
						this.gui.MidlineGrid, ...
						'Save midline' ...
					);
					this.initializeComponent(this.gui.MidlineGrid1_2);
					
					% + 2-2 Delete midline button
					this.gui.MidlineGrid2_2 = vt.Component.Button.DeleteMidline( ...
						this.gui.MidlineGrid, ...
						'Delete midline' ...
					);
					this.initializeComponent(this.gui.MidlineGrid2_2);
					
					% + 3-2 Cancel midline changes button
					this.gui.MidlineGrid3_2 = vt.Component.Button.CancelMidline( ...
						this.gui.MidlineGrid, ...
						'Cancel midline changes' ...
					);
